...
const resolveLocalidadId = async (provinceId: string, city?: string) => {
  ...
  const exact = localidades.find((item) => item.nombre.toLowerCase() === normalizedCity);
  if (exact) return exact;

  const partial = localidades.find(
    (item) =>
      item.nombre.toLowerCase().includes(normalizedCity) || normalizedCity.includes(item.nombre.toLowerCase())
  );
  return partial || localidades[0];
};
...
export const quoteShipping = async (input: ShippingQuoteInput): Promise<ShippingQuoteResult> => {
  ...
  for (const row of domicilioGroups.values()) {
    ...
    const sellerCost = await fetchSellerCost({
      provinceId,
      postalCode,
      weight,
      paquetes,
  ...
  const localidad = await resolveLocalidadId(provinceId, input.city);
  if (localidad) {
